...
本次提交前全量回归为 `331 passed、7 skipped、1 warning`；前端生产构建为 `58 modules transformed`，并通过 Python compileall、`docker compose config --quiet` 和 `git diff --check`。真实 Wan、InsightFace、MuseTalk 和外部 API 不在普通测试中自动调用。
