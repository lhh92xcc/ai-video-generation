...
- Vue 创作者前台与内部制作后台；根路径默认进入普通用户前台，后台“概览”提供真实运行摘要和快捷入口，Provider 配置是独立的系统页面；创作者工作区按“内容理解 / 剧本与分镜 / 媒体生成 / 审核与成片”四阶段导航，镜头清单支持状态筛选和关键词搜索，任务记录支持重复任务聚合；媒体资产支持授权预览、真实图片/视频缩略图、搜索和下载。
