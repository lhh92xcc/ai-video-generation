...
const canSubmitReview = computed(() => Boolean(selectedAsset.value && reviewer.value.trim() && reviewStatus.value !== selectedAsset.value.status && !submittingReview.value && !syncingAssets.value))
const reviewableAssetCount = computed(() => assets.value.filter((asset) => asset.status === 'needs_review').length)
const canSubmitBatchReview = computed(() => Boolean(projectId.value && reviewer.value.trim() && reviewableAssetCount.value > 0 && !submittingBatchReview.value && !syncingAssets.value))
...
async function syncAssets() {
  if (!projectId.value || !canEditAsset.value || syncingAssets.value || submittingReview.value || submittingBatchReview.value || savingAsset.value) return
